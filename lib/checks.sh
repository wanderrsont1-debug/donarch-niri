...
detect_aur_helper() {
    ...
    for helper in "${helpers[@]}"; do
        if command -v "$helper" &>/dev/null; then
            log_success "AUR helper detectado: $helper"
            echo "$helper"
            return 0
        fi
    done

    log_warn "Nenhum AUR helper encontrado (paru, yay, pikaur, pakku)."
    log_info "Alguns pacotes AUR não poderão ser instalados automaticamente."
    log_info "Para instalar o paru (recomendado), execute:"
    log_info "  sudo pacman -S --needed base-devel git"
    log_info "  git clone https://aur.archlinux.org/paru.git && cd paru && makepkg -si"
    echo "none"
    return 0
}
